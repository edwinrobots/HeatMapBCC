...
var mapdatastr = "mapdata/map";

function loadPeaks(layerName){
    //WHEN using on a server with proper AJAX so we can load a JSON file, 
    //comment out the below test data and uncomment the bits below.
//     if (layerName == "collapsed_buildings"){
//         data = eval("[[18.539269,-72.336408, 1],[18.543376,-72.338883, 0.5]]");
//     }else{
//         data = eval("[[18.539269,-71.336408, 0.2],[18.543376,-71.338883, 0.8]]");
//     }
    ...
    $.getJSON(mapdatastr+layerId+".json",  function(data){
        ...
        $.each( data, function(key,val){
            ...
            var lat = key/nx * (coords[1]-coords[0]) + coords[0];
            
            $.each( val, function(key, val){
            
                var lon = key/ny * (coords[3]-coords[2]) + coords[2];
        ...
        refreshHeatMap(peaksx, peaksy, peaksval); 
    });
...
function switchOverlay(){
    switchMap = $("input[name='addtrusted']:checked").attr("checked")
    val = $("input[name='meanstd']:checked").attr("id")
    if (switchMap){
        if (val=="mean"){
            mapdatastr = "mapdata/map_expert_"
        } else{
            mapdatastr = "mapdata/map_sd_expert_"
        }        
    } else {
        if (val=="mean"){
            mapdatastr = "mapdata/map"
        } else{
            mapdatastr = "mapdata/map_sd_"
        }
    }
...
function refreshHeatMap(peaksx, peaksy, peakVals){

    var peakPoints = [];
    
    for (i=0; i<peaksx.length; i++){
        var wloc = {
            location:new google.maps.LatLng(peaksx[i],peaksy[i]),
            weight:peakVals[i]
            
        };
        peakPoints.push(wloc);
    }
    
    var pointArray = new google.maps.MVCArray(peakPoints);
