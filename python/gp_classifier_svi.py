...
class GPClassifierSVI(GPClassifierVB):
    ...
    def __init__(self, dims, z0=0.5, shape_s0=None, rate_s0=None, s_initial=None, shape_ls=10, rate_ls=0.1, 
        ...
        super(GPClassifierSVI, self).__init__(dims, z0, shape_s0, rate_s0, s_initial, shape_ls, rate_ls, ls_initial, 
    ...
    def expec_s(self):
        ...
        self.old_s = self.s 
        invK_mm_expecFF = self.inv_Ks_mm_uS / self.s + (self.inv_Ks_mm/self.old_s).dot(self.um.dot(self.um.T))
    ...
    def choose_inducing_points(self):
        ...
        if self.ninducing > self.obs_coords.shape[0]:
            ninducing = self.obs_coords.shape[0]
        else:
        ...
        if not hasattr(self, 'prev_u_invSm') or self.prev_u_invSm is None:
            self.prev_u_invSm = np.zeros((ninducing, 1), dtype=float)# theta_1
            self.prev_u_invS = np.zeros((ninducing, ninducing), dtype=float) # theta_2

        mm_dist = np.zeros((ninducing, ninducing, len(self.dims)))
        nm_dist = np.zeros((nobs, ninducing, len(self.dims)))
